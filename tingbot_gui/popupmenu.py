...
class PopupMenu(ModalWindow):
    """A popupmenu"""
    def __init__(self,xy,align="center",style=None,cancellable=True,callback=None, menu_items=None):
        if style == None:
            style = get_default_style()
        size = (style.popupmenu_button_size[0],style.popupmenu_button_size[1]*len(menu_items))
        super(PopupMenu,self).__init__(xy,size,align,style,cancellable,callback)
        self.menu_items = menu_items
        
        #populate popupmenu
        for i, (label, callback) in enumerate(menu_items):
            but = PopupButton(xy = (0,i*self.style.popupmenu_button_size[1]),
